...
module.exports = {
	run: ( bot, message , args ) => {
	    ...
	    function category( args ) {
            message.guild.createChannel( args , "category", [{
            id: message.guild.id,
            allow: [ "READ_MESSAGES" , "READ_MESSAGE_HISTORY" ]
            }]);
        };
		
        function channel_create( args ) {            
            message.guild.createChannel( args , "text" );
        };
        /*function prompt(channel, options, sim, nao, nada) {
            channel.awaitMessages(msg => ['sim', 'não'].includes(msg.content), options)
                .catch(nada)
                .then((msgs) => {
                    var msg = msgs.first();
                    if(msg.content === 'sim') sim(msg)
                    else nao(msg)
                });
        }*/
        /*function wait_every_message(how_many_msg,time,do_what) {
            message.channel.awaitMessages(message.channel, { max: how_many_msg, time: time, errors: [ "time" ] }).then(c => {
                do_what
            });
        };*/
        ...
        /*prompt(message.channel, {max:1, time:10000, errors:[ "time" ]},
            (msg) => {
                message.reply("Qual vai ser o nome da categoria?");
            },
            (msg) => {
                message.reply("voce falou nao filha da puta")
            },
            (msg) => {
                message.reply("voce nao falou nada filha da puta")
            });*/
	},
