	...
	run: ( bot , message , args ) => {
		...
		try {
			if ( !args[ 0 ] ) {
				...
				embed.setTitle( frases[ Math.floor( Math.random() * ( frases.length ) ) ] );
				embed.addField( ":scroll: | Comandos" , helper.black_bg( files.join( ", " ) ).replace( /.js/gi, "" ) );
				embed.addField( ":pen_ballpoint: | Prefix aceitas" , helper.black_bg( config.prefixes.join( "` `" ) ) );
				if ( message.guild.iconURL ) {
					embed.setThumbnail( message.guild.iconURL );
				}
				message.channel.send( embed );
			} else if ( args[ 0 ] && files.find( s => args[ 0 ].toLowerCase().includes( s.replace( /.js/gi, "" ) ) ) ) {
				thing = require( `../comandos/${ args[ 0 ].toLowerCase() }.js` );
				embed.setAuthor( user.username, user.diaplayAvatarURL );
				embed.setColor( message.member.displayHexColor );
				embed.setTimestamp();
				embed.setFooter( "Zerinho6 Bot™ criado por Moru Zerinho6#6793" );
				embed.setImage( thing.photo );
				embed.setDescription( "Troque o ``p`` que aparece nas imagens para a prefix..exemplo: ``ze.avatar``." );
